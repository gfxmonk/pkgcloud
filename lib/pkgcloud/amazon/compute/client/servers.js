...
exports.renameServer = function renameServer(server, name, callback) {
  return this.setServerTags(server, {Name: name}, callback);
};

//
// ### function setServerTags(server, tags, callback)
// #### @server {Server|String} Server id or a server
// #### @tags   {Object} Tags
// #### @callback {Function} f(err).
//
// Sets server tags from the own properties of `tags`.
// This will override any existing tags with the same name.
//
exports.setServerTags = function setServerTags(server, tags, callback) {
  var serverId = server instanceof base.Server ? server.id : server;
  var pairs = [];
  Object.keys(tags).forEach(function(key) {
    pairs.push({Key: key, Value: tags[key]});
  });
  this.ec2.createTags({
    Resources: [serverId],
    Tags: pairs
...
//
// ### function getConsoleOutput(server, callback)
// #### @server {Server|String} Server id or a server
// #### @callback {Function} f(err, output).
//
// The output object has the following keys:
//  - output: a Buffer (or `null`)
//
exports.getConsoleOutput = function getConsoleOutput(server, callback) {
  var serverId = server instanceof base.Server ? server.id : server;
  this.ec2.getConsoleOutput({
    InstanceId: serverId,
  }, function(err, data) {
    ...
    return err
      ...
      : callback && callback(null, {
        output: output ? new Buffer(output, 'base64') : null
      })
  });
};
